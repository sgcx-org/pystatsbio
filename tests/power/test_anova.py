...
class TestPowerAnovaOneway:
    ...
    def test_solve_power(self):
        """Solve for power."""
        r = power_anova_oneway(n=50, effect_size=0.25, n_groups=3, alpha=0.05)
        assert 0.0 < r.power < 1.0

    def test_solve_f(self):
        """Solve for Cohen's f."""
        r = power_anova_oneway(n=50, n_groups=3, alpha=0.05, power=0.80)
        assert r.effect_size > 0

    def test_roundtrip(self):
        """Round-trip: solve n, then verify power >= target."""
        r1 = power_anova_oneway(effect_size=0.25, n_groups=3, alpha=0.05, power=0.80)
        r2 = power_anova_oneway(n=r1.n, effect_size=0.25, n_groups=3, alpha=0.05)
        assert r2.power >= 0.80

    def test_more_groups_more_n(self):
        """More groups require more subjects (per group)."""
        r3 = power_anova_oneway(effect_size=0.25, n_groups=3, alpha=0.05, power=0.80)
        r5 = power_anova_oneway(effect_size=0.25, n_groups=5, alpha=0.05, power=0.80)
    ...
    def test_k_less_than_2_error(self):
        with pytest.raises(ValueError, match="n_groups must be >= 2"):
            power_anova_oneway(effect_size=0.25, n_groups=1, alpha=0.05, power=0.80)
...
class TestPowerAnovaFactorial:
    ...
    def test_solve_n_main_effect(self):
        ...
        r = power_anova_factorial(
            effect_size=0.25, n_levels=(2, 3), alpha=0.05, power=0.80, effect="main-a",
        )
        assert isinstance(r.n, int)

    def test_solve_power(self):
        """Solve for power."""
        r = power_anova_factorial(n=30, effect_size=0.25, n_levels=(2, 3), alpha=0.05)
        assert 0.0 < r.power < 1.0

    def test_roundtrip(self):
        """Round-trip consistency."""
        r1 = power_anova_factorial(effect_size=0.25, n_levels=(2, 3), alpha=0.05, power=0.80)
        r2 = power_anova_factorial(n=r1.n, effect_size=0.25, n_levels=(2, 3), alpha=0.05)
        assert r2.power >= 0.80

    def test_invalid_effect(self):
        with pytest.raises(ValueError, match="effect"):
            power_anova_factorial(
                effect_size=0.25, n_levels=(2, 3), alpha=0.05, power=0.80, effect="invalid",
            )

    def test_invalid_n_levels(self):
        with pytest.raises(ValueError, match="at least 2 factors"):
            power_anova_factorial(effect_size=0.25, n_levels=(3,), alpha=0.05, power=0.80)

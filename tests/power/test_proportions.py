...
class TestPowerPropTest:
    ...
    def test_solve_n(self):
        ...
        r = power_prop_test(effect_size=h, alpha=0.05, power=0.80)
        assert isinstance(r.n, int)
        assert r.n > 0

    def test_solve_power(self):
        """Solve for power."""
        r = power_prop_test(n=100, effect_size=0.3, alpha=0.05)
    ...
    def test_roundtrip(self):
        ...
        r1 = power_prop_test(effect_size=0.3, alpha=0.05, power=0.80)
        r2 = power_prop_test(n=r1.n, effect_size=0.3, alpha=0.05)
        assert r2.power >= 0.80

    def test_larger_h_smaller_n(self):
        """Larger effect needs fewer subjects."""
        r1 = power_prop_test(effect_size=0.3, alpha=0.05, power=0.80)
        r2 = power_prop_test(effect_size=0.5, alpha=0.05, power=0.80)
        assert r2.n < r1.n

    def test_zero_h_error(self):
        """effect_size=0 when solving for n should raise."""
        with pytest.raises(ValueError, match="effect_size = 0"):
            power_prop_test(effect_size=0.0, alpha=0.05, power=0.80)
...
class TestPowerFisherTest:
    ...
    def test_solve_n(self):
        """Solve for n."""
        r = power_fisher_test(prop1=0.6, prop2=0.4, alpha=0.05, power=0.80)
        assert isinstance(r.n, int)
        assert r.n > 0

    def test_solve_power(self):
        """Solve for power."""
        r = power_fisher_test(n=100, prop1=0.6, prop2=0.4, alpha=0.05)
        assert 0.0 < r.power < 1.0

    def test_requires_p1_p2(self):
        """prop1 and prop2 must be provided."""
    ...
    def test_invalid_p1(self):
        ...
        with pytest.raises(ValueError, match="prop1"):
            power_fisher_test(n=100, prop1=1.5, prop2=0.3, alpha=0.05)
